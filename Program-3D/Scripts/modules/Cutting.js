    ...
    sendData(helperVertices){
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                let rf1 = [],rf2 = [];
                let rv1 = [],rv2 = [];
                let len1 = result.ResultF1.length;
                let len2 = result.ResultF2.length;
                ...
                for(let i = 0;i<len1;i++)
                {
                    ...
                    for(let j = 0;j < len;j++)
                    {
                        if(_.isEqual(result.ResultF1[i],rf1[j]))
                        {
                            flag = false;
                            break;
                        }
                    }
                    if(flag) rf1.push(result.ResultF1[i]);
                }
                for(let i = 0;i<len2;i++)
                {
                    ...
                    for(let j = 0;j < len;j++)
                    {
                        if(_.isEqual(result.ResultF2[i],rf2[j]))
                        {
                            flag = false;
                            break;
                        }
                    }
                    if(flag) rf2.push(result.ResultF2[i]);
                }
                scope.faces1 = rf1;
                scope.faces2 = rf2;
